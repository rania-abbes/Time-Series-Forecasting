...
The LSTM comes out ahead, but only once it's actually trained to convergence: an earlier version of this notebook capped training at a hardcoded 20 epochs, and the LSTM lost to both LightGBM and linear regression at that point (2.54°C test MAE). Adding early stopping (`src/training.py`) revealed validation loss was still improving at epoch 20 — it doesn't plateau until epoch ~62 (72 epochs run, 10-epoch patience). Retrained to that actual plateau, the LSTM's test MAE drops to 2.34°C, ahead of LightGBM. **The original "LSTM loses" conclusion was an artifact of under-training, not a real result about the task** — see the [Conclusions section](modeling.ipynb) of the notebook for the full before/after discussion. All three trained models beat the naive baselines by roughly 1-1.1°C of MAE, and the margin between the top three is itself fairly narrow (2.34-2.45°C).
